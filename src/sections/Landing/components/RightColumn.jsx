...
// --- SUB-COMPONENTS (Defined outside to avoid re-definition errors) ---
...
const ServiceCard = ({ service, currentLanguage, serviceDescriptions, onServiceClick, addToRefs }) => {
  ...
  useEffect(() => {
    if (service.gif) {
      const img = new Image();
      img.onload = () => setCurrentImage(service.gif); // English: Auto-switch to GIF once loaded
      img.src = service.gif;
    }
  }, [service.gif]);
  ...
  return (
    ...
    >
      ...
      {/* Image Area - Right side + Centered Vertically */}
...
const RightColumn = ({ onServiceClick, currentLanguage }) => {
  ...
  const handleTabSwitch = (tab) => {
    ...
    cellsRef.current = [] // English: Clear refs for new tab elements
    setActiveTab(tab)
  }

  const serviceHeaderContent = { en: serviceHeaderEnTxt, vi: serviceHeaderViTxt }
  const serviceDescriptions = {
    illustration: { en: serviceIllustrationEnTxt, vi: serviceIllustrationViTxt },
    gamedev: { en: serviceGamedevEnTxt, vi: serviceGamedevViTxt }
  }
  ...
  const servicesContent = {
    ...
    vi: [
      { buttonText: 'Animated Illustrations', sectionId: 'illustration', descriptionKey: 'illustration', image: serviceImg1, gif: serviceGif1, buttonColorNormal: '#ff69b4', buttonColorNormalEnd: '#ffcc00', buttonColorHighlight: '#f8d2d5', buttonColorHighlightEnd: '#fef3e2' },
      { buttonText: 'Game Development', sectionId: 'gamedev', descriptionKey: 'gamedev', image: serviceImg2, gif: serviceGif2, buttonColorNormal: '#7978e6', buttonColorNormalEnd: '#e99bba', buttonColorHighlight: '#cecef5', buttonColorHighlightEnd: '#e8f4f8' }
    ]
  }

  const services = servicesContent[currentLanguage]
  const collaborationContent = { en: collaborationEnTxt, vi: collaborationViTxt }
