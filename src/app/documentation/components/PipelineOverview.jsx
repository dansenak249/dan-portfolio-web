...
export default function PipelineOverview({ onSelect }) {
  return (
    <article>
      <h1 className="text-4xl font-bold leading-tight text-[#2d2d3a]">Vtuber 2D Pipeline</h1>
      <p className="mt-3 text-[17px] leading-relaxed text-[#3a3a52]">
        ...
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="bg-[#ebebeb]">
              <th className="w-1/4 border-r border-[#ebebeb] px-4 py-2 text-[12px] font-bold uppercase tracking-[0.12em] text-[#2d2d3a]">
                Stage
              </th>
              <th className="px-4 py-2 text-[12px] font-bold uppercase tracking-[0.12em] text-[#2d2d3a]">
                Description
              </th>
            </tr>
          </thead>
          <tbody>
            {pipeline.map((stage) => (
              <tr key={stage.id} className="border-t border-[#ebebeb] align-top">
                <td className="border-r border-[#ebebeb] px-4 py-2">
                  <button
                    type="button"
                    onClick={() => onSelect(stage.id)}
                    className="text-left text-base font-semibold text-[#5b4bd6] underline decoration-[#c4c4dd] underline-offset-2 transition-colors hover:text-[#ff69b4] hover:decoration-[#ff69b4]"
                  >
                    {stage.label}
                  </button>
                </td>
                <td className="px-4 py-2 text-base leading-relaxed text-[#2d2d3a]">{stage.blurb}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <section className="mt-8">
        <h2 className="text-2xl font-bold text-[#2d2d3a]">Additional resources</h2>
          ...
          {ADDITIONAL_RESOURCES.map((res) => (
            <li key={res.id} className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-base text-[#5b4bd6] underline decoration-[#c4c4dd] underline-offset-2 transition-colors hover:text-[#ff69b4] hover:decoration-[#ff69b4]"
              >
                {res.label}
              </button>
            </li>
